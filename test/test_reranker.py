...
import json, tqdm
import torch
from model import ClipRetriever, MistralAnswerGenerator, GPT4AnswerGenerator, reconstruct_wiki_article, PaLMAnswerGenerator, reconstruct_wiki_sections, WikipediaKnowledgeBaseEntry, BgeTextReranker
...
def run_test(test_file_path: str, knowledge_base_path: str, faiss_index_path:str, top_ks: list, retrieval_top_k: int, **kwargs):
    ...
    if kwargs['perform_vqa']:
        ...
        question_generator = MistralAnswerGenerator(model_path="/remote-home/share/huggingface_model/Mistral-7B-Instruct-v0.2", device="cuda:0", use_embedding_model=False)
    if kwargs['perform_text_rerank']:
        text_reranker = BgeTextReranker(model_path='/remote-home/share/huggingface_model/bge-reranker-v2-m3',device="cuda:0")
        eval_score = 0
    ...
    if kwargs['perform_qformer_reranker']:
        ...
        blip_model, vis_processors, txt_processors = load_model_and_preprocess(name="blip2_reranker", model_type="pretrain", is_eval=True, device="cuda")
    ...
    for it, test_example in tqdm.tqdm(enumerate(test_list)):
        ...
        image = PIL.Image.open(get_image(example["dataset_image_ids"].split("|")[0], example["dataset_name"], iNat_id2name))
        ...
        if kwargs["resume_from"] is not None:
            ...
            seen = set()
            retrieval_simlarities = [top_k[i]["similarity"] for i in range(retrieval_top_k) if not (top_k[i]["url"] in seen or seen.add(top_k[i]["url"]))]
            
       
        if kwargs["save_result"]:
            retrieval_result[data_id] = {"retrieved_entries": [entry.url for entry in entries[:20]], "retrieval_similarities": [sim.item() for sim in retrieval_simlarities[:20]]}
        ...
        if kwargs['perform_qformer_reranker']:
            ...
            with torch.cuda.amp.autocast():
                ...
                for section_spilit in range(0, len(qformer_articles), rerank_step): 
                    article_embs = blip_model.extract_features({
                        "text_input": qformer_articles[section_spilit:section_spilit+rerank_step]
                    }, mode="text")['text_embeds_proj'][:,0,:]
                    if section_spilit == 0:
                        article_embs_all = article_embs
                    else:
                        article_embs_all = torch.cat((article_embs_all, article_embs), dim=0)
                ...
                section_similarities = [retrieval_simlarities[section_to_entry[i]] for i in range(len(sections))]
                alpha_1 = 0.5
                alpha_2 = 1 - alpha_1
                scores = alpha_1 * torch.tensor(section_similarities).to("cuda") + alpha_2 * scores
            ...
            reranked_entries = remove_list_duplicates([entries[section_to_entry[i]] for i in reranked_index])
            reranked_sections = remove_list_duplicates([sections[i] for i in reranked_index])
            if kwargs["save_result"]:
                retrieval_result[data_id]["reranked_entries"] = [entry.url for entry in reranked_entries[:20]]
        ...
        if kwargs['perform_vqa']:
            answer = question_generator.llm_answering(question=example["question"], entry_section=reranked_sections[0])
