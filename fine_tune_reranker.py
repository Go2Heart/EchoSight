...
def finetune_blip(num_epochs: int, blip_model_name: str, learning_rate: float, batch_size: int,validation_frequency: int, transform: str, save_training: bool, save_best: bool,checkpoint_path: str = None, blip2_checkpoint_path: str = None, **kwargs):
    ...
    blip_model, vis_processors, txt_processors = load_model_and_preprocess(name=blip_model_name, model_type="pretrain", is_eval=False, device="cuda")
    ...
    if kwargs["eval_file"] is not None:
        ...
        eval_dataloader = DataLoader(eval_dataset, collate_fn=qformer_collate_fn, num_workers=kwargs['num_workers'], batch_size=batch_size, shuffle=True, drop_last=True)
    train_dataloader = DataLoader(train_dataset, pin_memory=True, collate_fn=qformer_collate_fn, num_workers=kwargs["num_workers"], batch_size=batch_size, shuffle=True, drop_last=True)
    ...
    scheduler = OneCycleLR(optimizer, max_lr=learning_rate, pct_start=1.5/num_epochs, div_factor=100., steps_per_epoch=len(train_dataloader), epochs=num_epochs)
    ...
    for epoch in range(num_epochs):
        ...
        blip_model.train()
        for idx, (reference_images, questions, positive_articles, negative_articles, _) in enumerate(train_bar):
            images_in_batch = reference_images.size(0)
            step = len(train_bar) * epoch + idx
            optimizer.zero_grad()
            reference_images = reference_images.to(device, non_blocking=True)
            import ipdb; ipdb.set_trace()
            positive_articles = [txt_processors["eval"](positive_article) for positive_article in positive_articles]
            negative_articles = [[txt_processors["eval"](article) for article in negative_article] for negative_article in negative_articles] # negative_articles is [batch_size, neg_num&2]
            ...
            with torch.cuda.amp.autocast():
                loss_dict = blip_model({"image":reference_images, "question":questions, "article":positive_articles, "negative": negative_articles})
                loss = 0.
                for key in loss_dict.keys():
                    loss += loss_dict[key]
            ...
            if save_training and (idx+1) % kwargs["save_frequency"] == 0:
                ...
                torch.save(blip_model.state_dict(), str(training_path / f'model_{epoch*len(train_dataloader) + idx}.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_frequency", default=50000, type=int, help="Save frequency expressed in steps")
    parser.add_argument("--checkpoint-path", default=None, type=str, help="Path to the checkpoint")
    parser.add_argument("--blip2-checkpoint-path", default=None, type=str, help="Path to the blip2 checkpoint")
    parser.add_argument("--train_file", type=str, help="Path to the training file")
    parser.add_argument("--eval_file", default=None, type=str, help="Path to the evaluation file")
    parser.add_argument("--knowledge_base_file", type=str, help="Path to the knowledge base file")
    parser.add_argument("--negative_db_file", type=str, help="Path to the negative db file")
    parser.add_argument("--inat_id2name", type=str, help="Path to the inat_id2name.json")
